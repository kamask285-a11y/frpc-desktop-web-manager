...
将 Node.js 项目放在项目根目录下（默认 `~/Web`，页面上“项目目录”旁的“修改”可
随时改成任意绝对路径）。一级项目目录包含 `package.json` 时即可被识别。已管理的
项目不会因为修改项目目录而失效。在侧栏打开“Web 服务”，可以扫描项目、选择 npm
...
这些值没有内置默认值，需要按自己的服务器填写；公网 IP 与基础域名是计算域名和远程
端口的前提，未填写时应用会提示先完成服务器设置。
