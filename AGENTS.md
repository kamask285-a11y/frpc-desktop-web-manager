...
- 本项目源码已下载到当前仓库根目录，依赖已安装，源码构建已验证成功。
- 已生成 macOS universal 构建，并用于替换本机原有的 `/Applications/Frpc-Desktop.app`。
- 原应用已备份为 `/Applications/Frpc-Desktop.app.backup-20260912`，原有用户配置已保留。
- 当前运行的是源码构建版本，FRP 状态正常，版本为 `1.2.7`。
- 后续开发、编译、运行和验证均以当前源码为准；涉及替换应用时必须保留用户已有配置。
